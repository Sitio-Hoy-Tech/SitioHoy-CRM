...
export default function NuevoArticuloPage() {
  ...
  useEffect(() => {
    fetch("/api/kb/categorias").then(r => r.json()).then(j => setCategorias(j.data || []));
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSaving(true);
    const res = await fetch("/api/kb/articulos", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    router.push(`/base-conocimientos/${json.data.id}`);
  }
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-heading">Nuevo artículo</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-card rounded-xl border border-edge p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Título" required autoFocus
              ...
              placeholder="Ej: Cómo crear un cliente nuevo"
            />
        ...
        <div className="flex justify-between">
          ...
          <Button type="submit" loading={saving}>Publicar artículo</Button>
        </div>
      </form>
